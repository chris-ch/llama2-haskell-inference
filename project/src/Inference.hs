    ...
    where
      ...
      go network logits indexToken result token sakv
        ...
        | indexToken < length promptTokens = do
          let currentToken = promptTokens !! indexToken
          liftIO $ printf "%s" $ BSC.unpack (vocab !! fromIntegral currentToken)
          liftIO $ hFlush stdout
          go network logits (indexToken + 1) (result ++ [vocab !! fromIntegral currentToken]) (promptTokens !! (indexToken + 1)) sakv
        | otherwise = do
